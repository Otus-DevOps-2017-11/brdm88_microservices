...
Для его изменения необходимо задать значение переменной среды `COMPOSE_PROJECT_NAME`, либо же использовать ключ `-p` при запуске docker-compose, 
например, `docker-compose -p <project_name> <command>`.

###### .override-файл

Создан файл `docker-compose.override.yml`, реализующий:
