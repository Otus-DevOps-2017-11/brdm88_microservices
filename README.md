...
Созданы Docker-машины для хоста `master` и двух хостов `worker`, развернут кластер **Docker Swarm**.
Приложение Reddit запущено в кластере, настроены ограничения размещения. Опробованы варианты масштабирования.

При добавлении в кластер дополнительной `worker`-машины на ней запустится только контейнеры сервисов, у которых стоит режим деплоя `global`, 
...
Собственно, видно, что replicated сервисы балансируются по доступным ресурсам. При наличии свободных нод, они будут использованы в первую очередь.

Рассмотрена работа Routing Mesh.
